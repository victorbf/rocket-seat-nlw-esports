...
export const CreateAdModal: React.FC = () => {
  ...
  const handleCreateAd = handleSubmit(async values => {
    try {
      await axios.post(`/games/${values.gameId}/ads`, values);
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="bg-shape bg-[#2A2634] fixed top-1/2 left-1/2 max-h-screen w-full max-w-[480px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-sm py-8 px-10 text-white shadow-lg shadow-black/25 sm:rounded-lg">
